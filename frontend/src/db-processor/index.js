...
const DBMigrator = () => {
  ...
  const handleDBCheck = async (db, setDatabase) => {
    try {
      ...
      const response = await fetch(
        `${baseURL}/tools/db-processor/checkconnect`,
        {
          method: "POST",
          headers: {
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${baseURL}/tools/db-processor/db-analyze`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    ...
    ${isLoading ? "opacity-50 cursor-not-allowed" : ""}`}
                ...
                <button
                  type="submit"
                  className="flex items-center px-6 py-2 text-white transition-transform duration-150 ease-in-out transform bg-gray-800 rounded hover:scale-105 active:scale-95 focus:outline-none hover:bg-gray-700"
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="h-full p-6 bg-white rounded-lg shadow-md">
            <pre className="h-full p-2 bg-gray-100 rounded">
